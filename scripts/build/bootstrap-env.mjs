...
// The file holds STORAGE_ENCRYPTION_KEY, JWT_SECRET and API_KEY_SECRET, so it
// must never be world/group readable. `mode` only applies when the file is
// created; chmod afterwards to also tighten a pre-existing looser file. On
// Windows the POSIX mode is a no-op — swallow any chmod error there.
