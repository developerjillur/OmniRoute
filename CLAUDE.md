...
<!-- NEXA-LIVING-FORK — keep this block; on an upstream merge conflict, KEEP BOTH sides -->

> ⚠️ **This is a NexaLance LIVING FORK of `diegosouzapw/OmniRoute`.** It carries local
> customizations (security-audit + QA + perf + NexaConnect config, ~59 files) on branch
> `nexalance`. Remotes: `origin` = our fork (`developerjillur/OmniRoute`, push here), `upstream`
> = the original (never push). To **pull upstream updates, resolve a merge conflict, upgrade the
> OmniRoute version, contribute a fix upstream, or if the standalone won't boot after a build →
> use the `nexa-fork-sync` skill** (`.claude/skills/nexa-fork-sync/`) or dispatch the
> `nexa-fork-syncer` agent. It has the full merge→validate→land→deploy workflow, the hard
> invariants (never push to upstream; keep BOTH sides on conflicts; validate before landing), and
> the gotcha catalog. Quick check: `npm run nexa:status`. Runbook: `scripts/nexa-sync/README.md`.
