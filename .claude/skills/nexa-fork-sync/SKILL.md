...
5. **Housekeep** → re-baseline tsc-ratchet (upstream's TS-error delta ≠ our regression) — the ratchet is **down-only**, so `--update` will NOT raise it; when upstream RAISED the count, hand-edit `value` in `config/quality/tsc-error-baseline.json` (confirm our code added 0 via a stash-compare first) + `gen-manifest.mjs`; update the mirror `git branch -f upstream-main upstream/main`; commit.
6. **Push** → `git push origin nexalance`.
7. **Deploy** → `npm install` (sync deps to the merged lockfile) → back up (**Anchor rollbacks**) → `npm run build`. **Smoke the built artifact OUT-OF-BAND FIRST** (throwaway port + data dir, live :28128 untouched): `(cd .build/next/standalone && PORT=28129 DATA_DIR=/tmp/omni-smoke node dev/run-standalone.mjs)` — **you MUST `cd` into the standalone dir first** (Next's `server.js` resolves relative to CWD; running it from the repo root crashes with `Cannot find module …/server.js` = health `000`, exactly how the live launchd service runs it). The build relocates `scripts/dev/run-standalone.mjs` → `dev/run-standalone.mjs`. Then check `/api/monitoring/health`, a `cx/gpt-5.5` completion, `x-omniroute-catalog-cache` + `x-omniroute-*` streaming headers, CORS fail-closed. (Kill the throwaway by port — `kill $(lsof -ti tcp:28129)` — not by PID; `DATA_DIR`/`PORT` are env vars, so pkill patterns won't match.) **Only if the throwaway passes**, flip live: `launchctl kickstart -k gui/$(id -u)/com.nexalance.omniroute-test` → re-smoke :28128 → watch `/tmp/omniroute-28128.log` ~10 min (launchd KeepAlive can mask a crash loop). **Rollback:** fast = `rm -rf .build/next/standalone && mv .build/next/standalone.bak .build/next/standalone && launchctl kickstart -k …`; source = `git reset --hard nexa-prelease-<stamp>` → rebuild → kickstart.
...
- **tsc-ratchet fails after an upstream merge:** upstream may add TS errors — that's THEIRS, not our regression. The ratchet is **down-only**, so `--update` will NOT raise the baseline — hand-edit `value` in `config/quality/tsc-error-baseline.json` to the new count (v3.8.45 raised 2965→3014). The app still builds regardless (`next.config` `ignoreBuildErrors:true`); this is only a quality gate. (`--update` is what you run when the count DROPS.)
